...
symlinkSync(join(root, "node_modules"), join(built, "node_modules"), "junction");
for (const directory of ["investigation", "structured-output", "tools"]) {
	...
	for (const name of readdirSync(join(core, directory)).filter((file) => file.endsWith(".ts"))) {
		...
		const output = transformSync(source, { loader: "ts", target: "es2022", format: "esm", sourcefile: name });
		writeFileSync(join(built, directory, name.replace(/\.ts$/, ".js")), output.code);
	}
}
...
} = await load("investigation/common.js");
const { parseInput, parseLimits, parseProfile, effectivePolicy, createSecretScreen, validateEndpoint } =
	await load("investigation/policy.js");
...
const { parseStrictJson, parseDeciderResponse, requestDecider, choiceQuestions } = await load(
	"structured-output/decider-transport.js",
);
...
const PROFILE = Object.freeze({
	id: "fixture-only",
	approved: true,
	minTopProbability: 0.7,
	minTopTwoMargin: 0.2,
	backendId: "decider/fixture",
	deploymentFingerprint: "a".repeat(64),
	weightsRevision: "fixture-weights",
	...
	evaluationId: "fixture-not-production",
});
...
const backend = (extra = {}) => ({
	...
	token: "fixture-not-real-service-token-123456789",
	profile: PROFILE,
	limits: MAXIMUM_LIMITS,
	isSafe: () => true,
	...extra,
});
const policy = (extra = {}, check = () => {}, screen = () => true) =>
	effectivePolicy({ profile: PROFILE, scopePaths: ["."], ...extra }, check, screen);
const trace = () => {
	const events = [];
	return {
		id: "fixture-trace",
		events,
		write(event, data) {
			events.push({ event, ...structuredClone(data) });
		},
	};
};
function scope(names = ["src/a.ts"]) {
	return {
		id: "fixture-scope",
		revision: "fixture-revision",
		entriesEnumerated: names.length,
		files: Object.fromEntries(names.map((name) => [name, { version: `v:${name}`, size: 8 }])),
		restrictions: { readOnly: true, lateRestriction: "no writes" },
	};
}
function response(ids, selected = ids[0], values = ids.map((_, i) => (i === 0 ? 1 : 0))) {
	const probs = Object.fromEntries(ids.map((id, i) => [id, values[i]]));
	return {
		model: PROFILE.backendId,
		deploymentFingerprint: PROFILE.deploymentFingerprint,
		compatibility,
		usage: { input_tokens: 123, output_tokens: 0 },
		answers: {
			next: { type: "choice", choice: selected, confidence: probs[selected], certainty: 0.5, probabilities: probs },
		},
	};
}
function selection(step, selected, values) {
	...
	const parsed = parseDeciderResponse(
		response(ids, chosen, ps),
...
function mockRepository(s = scope(), options = {}) {
	return {
		...
		prepare: async () => s,
		assertFresh: async () => {},
		accounting: () => ({ entriesEnumerated: s.entriesEnumerated, bytesScanned: 0 }),
		async execute(candidate, context) {
			context.markDispatched();
			this.dispatched++;
			return candidate.action.kind === "read_range"
				? {
						kind: "source_excerpt",
						path: candidate.action.path,
						startLine: candidate.action.startLine,
						endLine: candidate.action.startLine,
						coveredEndLine: candidate.action.startLine,
						...
						sources: [
							{
								path: candidate.action.path,
								version: s.files[candidate.action.path].version,
								contentHash: hash("fact"),
								bytesRead: 4,
							},
						],
					}
...
async function run(input, overrides = {}) {
	...
	const deps = {
		...
		trace: audit,
		now: () => performance.now(),
		decide: async (step) => selection(step),
		...overrides,
	};
...
test("input is closed, UTF-8 bounded, and cannot carry host policy", () => {
	for (const input of [
		{ objective: "x", endpoint: "http://remote" },
		{ objective: "x", budget: 6 },
		{ objective: "😀".repeat(513) },
		{ objective: "x", diagnosticText: "x".repeat(8193) },
		{ objective: "x", literalTerms: ["x"] },
		{ objective: "x", literalTerms: Array(9).fill("xx") },
		{ objective: "x", seedLocations: [{ path: "src/a.ts", line: 0 }] },
	])
		assert.throws(() => parseInput(input, MAXIMUM_LIMITS), toolError("invalid_input"));
	for (const path of [
		"../secret",
		"/etc/passwd",
		"src/../a.ts",
		...
		assert.throws(
			() => parseInput({ objective: "x", seedLocations: [{ path }] }, MAXIMUM_LIMITS),
			toolError("permission_denied"),
		);
	const valid = parseInput(
		{ objective: "test", literalTerms: ["😀😀"], seedLocations: [{ path: "./src/a.ts" }] },
		MAXIMUM_LIMITS,
	);
	assert.equal(valid.seedLocations[0].path, "src/a.ts");
	assert.ok(Object.isFrozen(valid));
});
test("host limits can only decrease and zero is invalid", () => {
	for (const [key, max] of Object.entries(MAXIMUM_LIMITS))
		for (const n of [0, -1, max + 1, 1.1, Infinity])
...
test("admission profile has no threshold defaults and binds exact versions", () => {
	...
	for (const key of ["minTopProbability", "minTopTwoMargin", "deploymentFingerprint", "temperature", "evaluationId"]) {
		const copy = { ...PROFILE };
		delete copy[key];
		assert.throws(() => parseProfile(copy), toolError("host_configuration"));
	}
...
test("endpoint admits only literal loopback with no credentials, query, or redirect destination", () => {
	for (const endpoint of [
		...
		"http://127.0.0.1@evil/v1/systemone",
		"http://127.0.0.1/v1/systemone?x=y",
...
test("candidate generation is deterministic, deduplicated, complete and factual", () => {
	const s = scope(["src/b.ts", "src/a.ts"]);
	const p = policy();
	const input = {
		objective: "find evidence",
		diagnosticText: "at src/b.ts:100:1",
		seedLocations: [{ path: "src/a.ts" }, { path: "src/b.ts", line: 100 }],
		literalTerms: ["abc", "abc"],
	};
	const a = new CandidateBuilder(input, s, p).build();
	const b = new CandidateBuilder(input, s, p).build();
	assert.deepEqual(a, b);
	assert.equal(a.length, 3);
	assert.equal(a[0].provenance.source, "diagnostic");
	assert.deepEqual(a[0].action, { kind: "read_range", path: "src/b.ts", startLine: 70, endLine: 189 });
	assert.equal(a[1].action.path, "src/a.ts");
	assert.equal(a[2].action.term, "abc");
	assert.ok(Object.isFrozen(a[0].action));
	assert.ok(a.every((item) => !/fixes|proves/.test(item.description)));
});
test("unchanged operations and fully covered ranges are removed", () => {
	const s = scope();
	const builder = new CandidateBuilder(
		{ objective: "x", seedLocations: [{ path: "src/a.ts" }, { path: "src/a.ts", line: 2 }] },
		s,
		policy(),
	);
...
test("zero candidates performs no decision or content access", async () => {
	...
	assert.equal(result.counters.decisionRequests, 0);
	assert.equal(result.counters.actionsDispatched, 0);
});
test("one candidate is sent with handoff, not executed automatically", async () => {
	let calls = 0;
	const { result } = await run(
		{ objective: "x", seedLocations: [{ path: "src/a.ts" }] },
		{
			decide: async (step) => {
				calls++;
				assert.equal(step.candidates.length, 1);
				assert.equal(Object.keys(choiceQuestions(step.candidates).next.criteria).length, 2);
				return selection(step, "handoff");
			},
		},
	);
	assert.equal(calls, 1);
	assert.equal(result.reason, "decider_handoff");
	assert.equal(result.counters.actionsDispatched, 0);
});
test("33 candidates hand off without tournament or provider request", async () => {
	const names = Array.from({ length: 33 }, (_, i) => `src/f${i}.ts`);
	const { result } = await run(
		{ objective: "x", diagnosticText: names.map((name) => `${name}:1:1`).join("\n") },
		{
			repository: mockRepository(scope(names)),
			decide: () => assert.fail("provider called"),
		},
	);
	assert.equal(result.reason, "candidate_overflow");
	assert.equal(result.remainingCandidates, 33);
	assert.equal(result.omitted.candidates, 33);
});
test("tie and both uncertainty gates refuse dispatch", async () => {
	for (const values of [
		...
		const { result } = await run(
			{ objective: "x", seedLocations: [{ path: "src/a.ts" }] },
			{ decide: async (step) => selection(step, undefined, values) },
		);
	...
	const { result } = await run(
		{ objective: "x", seedLocations: [{ path: "src/a.ts" }] },
		{
			policy: policy({ profile: { ...PROFILE, minTopTwoMargin: 0.9 } }),
			decide: async (step) => selection(step, undefined, [0.8, 0.2]),
		},
	);
	assert.equal(result.reason, "uncertain");
});
test("at most five actions and five decisions, with no extra exhausted-budget decision", async () => {
	...
	const { result } = await run(
		{ objective: "x", seedLocations: names.map((path) => ({ path })) },
		{ repository: mockRepository(scope(names)) },
	);
	assert.equal(result.reason, "action_limit");
	assert.equal(result.counters.actionsDispatched, 5);
	assert.equal(result.counters.decisionRequests, 5);
...
test("two consecutive operations without new evidence or candidates stop", async () => {
	...
	assert.equal(result.counters.actionsDispatched, 2);
});
...
test("malformed or mismatched injected decisions never dispatch", async () => {
	for (const change of [
		...
		(s) => {
			s.value.actionId = "not-listed";
		},
		(s) => {
			s.usage.inputTokens = NaN;
		},
		(s) => {
			s.evidence.deploymentFingerprint = "c".repeat(64);
		},
		(s) => {
			s.evidence.topProbability = 0.1;
		},
		(s) => {
			s.evidence.probabilities.handoff = 1;
		},
	]) {
		const { result } = await run(
			{ objective: "x", seedLocations: [{ path: "src/a.ts" }] },
		...
		assert.ok(["decision_invalid", "model_mismatch"].includes(result.reason));
		assert.equal(result.counters.actionsDispatched, 0);
	}
});
test("Decider rounded uniform 3/33 options normalize; excess discrepancy rejects", () => {
	for (const n of [3, 33]) {
		...
		const questions = {
			next: { instructions: "choose", criteria: Object.fromEntries(ids.map((id) => [id, "read"])) },
		};
		...
		assert.equal(parsed.evidence.normalization, "decider-four-decimal");
		assert.equal(parsed.evidence.topTwoMargin, 0);
		assert.ok(Math.abs(Object.values(parsed.evidence.probabilities).reduce((a, b) => a + b, 0) - 1) < 1e-12);
		assert.throws(
			() =>
				parseDeciderResponse(
					response(
						ids,
						ids[0],
						values.map((p) => p + 0.0001),
					),
					questions,
					backend(),
					"set-hash",
...
test("strict parser rejects missing, extra, duplicate, nonfinite, nonmaximal, and invalid usage", () => {
	...
	for (const mutate of [
		(r) => {
			delete r.answers.next.confidence;
		},
		(r) => {
			r.extra = 1;
		},
		(r) => {
			r.answers.next.probabilities.extra = 0;
		},
		(r) => {
			r.answers.next.choice = "handoff";
			r.answers.next.confidence = 0;
		},
		(r) => {
			r.usage.output_tokens = -1;
		},
		(r) => {
			r.answers.next.probabilities.a = Infinity;
		},
	]) {
		const r = response(["a", "handoff"]);
		mutate(r);
		assert.throws(() => parseDeciderResponse(r, qs, backend(), "hash"), reason("decision_invalid"));
	}
});
test("full wire request overflow is rejected without fetch and restrictions are not truncated", async () => {
	...
	const client = backend({
		fetch: async (_url, init) => {
			...
			assert.equal(body.state.lastRestriction, "do not change permissions");
			assert.equal(init.redirect, "error");
			return new Response(JSON.stringify(response(["a", "handoff"])));
		},
	});
	await requestDecider(
		{ objective: "find", lastRestriction: "do not change permissions" },
		qs,
		"instructions",
		client,
		"hash",
	...
	await assert.rejects(
		requestDecider({ text: "x".repeat(32769) }, qs, "i", client, "hash", new AbortController().signal),
		reason("context_limit"),
	);
	await assert.rejects(
		requestDecider({ text: "x" }, qs, "i".repeat(65536), client, "hash", new AbortController().signal),
		reason("context_limit"),
	);
	assert.equal(calls, 1);
});
test("body bounds, duplicate HTTP response keys and model mismatch cannot admit a decision", async () => {
	...
	for (const body of [
		" ".repeat(1048577),
		'{"model":"x","model":"y"}',
		JSON.stringify({ ...response(["a", "handoff"]), deploymentFingerprint: "wrong" }),
	]) {
		await assert.rejects(
			requestDecider(
				{ objective: "x" },
				qs,
				"i",
				backend({ fetch: async () => new Response(body) }),
				"hash",
				new AbortController().signal,
			),
		);
...
test("unsafe input never reaches the provider or trace text", async () => {
	...
	const { result } = await run(
		...
		{
			trace: audit,
			policy: policy({}, () => {}, createSecretScreen([sensitive])),
			decide: () => assert.fail("called"),
		},
	);
	assert.equal(result.reason, "input_context_unsafe");
	assert.ok(!JSON.stringify(audit.events).includes(sensitive));
});
...
test("shared deadline includes preparation and caps an uncooperative provider", async () => {
	const { result } = await run(
		{ objective: "x", seedLocations: [{ path: "src/a.ts" }] },
		{
			policy: policy({ limits: { totalTimeoutMs: 20, decisionTimeoutMs: 15 } }),
			decide: () => new Promise(() => {}),
		},
	);
	...
	const second = await run(
		{ objective: "x", seedLocations: [{ path: "src/a.ts" }] },
		{ policy: policy({ limits: { totalTimeoutMs: 50 } }), now: () => now, repository },
	);
	assert.equal(second.result.reason, "deadline");
	assert.equal(second.result.counters.decisionRequests, 0);
});
test("policy change across an await is denied, and audit failure prevents dispatch", async () => {
	let current = true;
	const p = policy({}, () => {
		if (!current) throw new InvestigationToolError("permission_denied");
	});
	await assert.rejects(
		run(
			{ objective: "x", seedLocations: [{ path: "src/a.ts" }] },
		...
		run(
			{ objective: "x", seedLocations: [{ path: "src/a.ts" }] },
			{
				...
				trace: {
					id: "x",
					write(event) {
						if (event === "action-start") throw new Error("disk failure");
					},
				},
			},
		),
		toolError("audit_failed"),
	);
...
test("source changes before action result prevent evidence admission", async () => {
	...
	const repository = mockRepository(scope(), {
		assertFresh: async () => {
			if (++checks >= 3) throw new Handoff("source_changed");
		},
	});
...
function realFixture(files, overrides = {}) {
	const temp = mkdtempSync(join(tmpdir(), "atomic-decider-fs-"));
	...
	const audit = trace();
	const p = overrides.policy ?? policy();
	const calls = [];
	const repository = new LinuxRepositoryFacade({
		root: repo,
		policy: p,
		trace: audit,
		dispatch:
			overrides.dispatch ??
			(async (call, execute, signal) => {
...
test("real read returns exact UTF-8 text, LF line numbers, hashes and metered access", { skip: !linux }, async () => {
	...
	try {
		...
		assert.equal(fixture.calls[0].toolName, "read");
		assert.equal(fixture.calls[0].parentInvocationId, "parent-call");
		assert.deepEqual(fixture.calls[0].args, { path: "src/a.ts:1-120" });
		assert.equal(
			fixture.audit.events.filter((e) => e.event === "content-access").reduce((a, e) => a + e.bytes, 0),
...
test("literal search is not a regex and produces bounded actual-location read candidates", {
	skip: !linux,
}, async () => {
	...
	try {
		...
		assert.deepEqual(
			text.matches.map((m) => m.line),
			[1, 3],
		);
		assert.equal(text.scopeFullyScanned, true);
		assert.deepEqual(fixture.calls[0].args, { pattern: "a\\.b", paths: ["src/a.ts"], i: false, gitignore: false });
	} finally {
		fixture.close();
	}
});
test("excluded names, symlinks, special files and oversized targets cannot become eligible", {
	skip: !linux,
}, async () => {
	const fixture = realFixture({
		"ok.ts": "fine",
		".env": "SECRET",
		"node_modules/x.ts": "hidden",
		"big.ts": Buffer.alloc(1048577),
	});
	try {
...
test("symlink replacement during decision prevents content dispatch", { skip: !linux }, async () => {
	...
	try {
		const { result } = await run(
			{ objective: "x", seedLocations: [{ path: "a.ts" }] },
			{
				...fixture,
				decide: async (step) => {
					rmSync(join(fixture.repo, "a.ts"));
					symlinkSync("/etc/passwd", join(fixture.repo, "a.ts"));
					return selection(step);
				},
			},
		);
...
test("canonical hook denial is a tool error and precedes every content access", { skip: !linux }, async () => {
	...
	try {
		await assert.rejects(
			run({ objective: "x", seedLocations: [{ path: "a.ts" }] }, fixture),
			toolError("permission_denied"),
		);
...
test("binary reads fail safely, read bytes are capped, and incomplete searches report unknown omissions", {
	skip: !linux,
}, async () => {
	const binary = realFixture({ "binary.txt": Buffer.from([65, 0, 66]) });
	try {
		const { result } = await run({ objective: "x", seedLocations: [{ path: "binary.txt" }] }, binary);
		assert.equal(result.reason, "operation_failed");
		assert.equal(result.evidence.length, 0);
	} finally {
		binary.close();
	...
	try {
		const { result } = await run({ objective: "x", seedLocations: [{ path: "large.ts" }] }, large);
		assert.equal(Buffer.byteLength(result.evidence[0].text), 6144);
		assert.equal(result.evidence[0].bounded, true);
		assert.equal(result.omitted.evidenceBytes, 5856);
	} finally {
		large.close();
	}
	...
	try {
		const { result } = await run(
			{ objective: "x", literalTerms: ["term"] },
			{
				...search,
				decide: async (step) =>
					selection(step, step.candidates[0].action.kind === "search_literal" ? undefined : "handoff"),
			},
		);
		assert.equal(JSON.parse(result.evidence[0].text).matches.length, 20);
		assert.equal(result.omitted.matches, null);
	} finally {
		search.close();
	}
});
test("scan and enumeration budgets include preparation and all dispatched scans", { skip: !linux }, async () => {
	...
	try {
		...
		assert.equal(result.omitted.matches, null);
		assert.equal(fixture.repository.accounting().bytesScanned, 5);
	} finally {
		fixture.close();
	}
	const capped = realFixture(
		{ "a.ts": "a", "b.ts": "b", "c.ts": "c" },
		{ policy: policy({ limits: { maxEnumeratedEntries: 2 } }) },
	);
	try {
		const { result } = await run({ objective: "x", literalTerms: ["term"] }, capped);
		assert.equal(result.reason, "context_limit");
		assert.equal(result.counters.decisionRequests, 0);
		assert.equal(capped.repository.accounting().entriesEnumerated, 2);
	} finally {
		capped.close();
	}
});
test("durable traces retain evidence once and never resume incomplete work", { skip: !linux }, async () => {
	...
	try {
		...
		await run({ objective: "x", seedLocations: [{ path: "a.ts" }] }, { ...fixture, trace: audit });
		audit.close();
		const stored = inspectTrace(audit.path);
		...
		incomplete.write("invocation-start", { status: "incomplete" });
		incomplete.close();
		assert.equal(inspectTrace(incomplete.path).status, "incomplete");
		assert.throws(
			() => new FileTraceSink(fixture.repo, fixture.repo, "unsafe", () => true),
			toolError("audit_failed"),
		);
		const link = join(fixture.temp, "audit-link");
		symlinkSync(fixture.repo, link);
...
test("unapproved repositories and repository-local files cannot activate the feature", { skip: !linux }, () => {
	...
	const config = {
		experimental: {
			deciderInvestigation: {
				...
				profile: { ...PROFILE, evaluationId: "heldout-test-fixture" },
				tokenSecretRef: "env:ATOMIC_DECIDER_SERVICE_TOKEN",
				approvedRepositories: [{ root: fixture.repo, scopePaths: ["."] }],
			},
		},
	};
	...
	try {
		...
		const host = readHostConfiguration(path, fixture.repo);
		assert.equal(host.root, fixture.repo);
		assert.equal(host.endpoint, config.experimental.deciderInvestigation.endpoint);
		assert.equal(readHostConfiguration(path, fixture.auditDir), undefined);
		const inRepo = join(fixture.repo, "settings.json");
		writeFileSync(inRepo, JSON.stringify(config), { mode: 0o600 });
		...
		config.experimental.deciderInvestigation.tokenSecretRef = "env:TYPESAFE_API_KEY";
...
test("session concurrency rejects a second invocation without queueing; fresh explicit invocation is allowed", () => {
	...
	const other = acquireInvestigationSession("session-two");
	other();
	release();
	release();
...
test("shared canonical hooks see actual arguments, parent IDs, failures, and filtered results cannot be bypassed", async () => {
	...
	const runner = {
		...
		async emitToolCall(event) {
			seen.push(event);
			return mode === "deny" ? { block: true } : undefined;
		},
	...
	const dispatch = createInvestigationDispatcher(
		{ _agentEventQueue: Promise.resolve(), _extensionRunner: runner },
		() => {},
	);
	const call = {
		toolName: "read",
		...
		args: { path: "src/a.ts:1-120" },
	};
	const execute = async () => {
		executions++;
		if (mode === "failure") throw new Handoff("operation_failed");
		return { kind: "source_excerpt", text: "original", sources: [] };
	};
	const signal = new AbortController().signal;
	assert.equal((await dispatch(call, execute, signal)).text, "original");
	assert.equal(seen[0].toolName, "read");
	assert.equal(seen[0].parentInvocationId, "parent-1");
	assert.deepEqual(seen[0].input, call.args);
	mode = "deny";
	await assert.rejects(dispatch(call, execute, signal), toolError("permission_denied"));
	assert.equal(executions, 1);
	mode = "filter";
	await assert.rejects(dispatch(call, execute, signal), reason("operation_failed"));
	mode = "mutate";
	await assert.rejects(dispatch(call, execute, signal), reason("operation_failed"));
	mode = "failure";
	await assert.rejects(dispatch(call, execute, signal), reason("operation_failed"));
	assert.equal(seen.at(-1).isError, true);
});
test("root or directory replacement is detected across decision boundaries", { skip: !linux }, async () => {
	...
	try {
		const { result } = await run(
			{ objective: "x", seedLocations: [{ path: "sub/a.ts" }] },
			{
				...fixture,
				decide: async (step) => {
					renameSync(join(fixture.repo, "sub"), join(fixture.repo, "old"));
					mkdirSync(join(fixture.repo, "sub"));
					writeFileSync(join(fixture.repo, "sub/a.ts"), "replacement");
					return selection(step);
				},
			},
		);
...
test("source secrets are stopped before result hooks, provider transmission, or evidence persistence", {
	skip: !linux,
}, async () => {
	const value = "configured-sensitive-value-123456789";
	const fixture = realFixture({ "a.ts": value }, { policy: policy({}, () => {}, createSecretScreen([value])) });
	try {
		const { result } = await run({ objective: "x", seedLocations: [{ path: "a.ts" }] }, fixture);
		assert.equal(result.reason, "input_context_unsafe");
		assert.equal(result.evidence.length, 0);
		assert.ok(!JSON.stringify(fixture.audit.events).includes(value));
	} finally {
		fixture.close();
	}
});
...
test("Jev authentication, response validation, partitioning and tournament function tokens are unchanged", () => {
	...
	const source = parse(text, { sourceType: "module", plugins: ["typescript"] });
	const expected = JSON.parse(
		readFileSync(join(root, "scripts/decider-investigation-evaluation/fixtures/jev-function-hashes.json"), "utf8"),
	);
	for (const [name, digest] of Object.entries(expected)) {
		const node = source.program.body.find(
			(item) =>
				(item.declaration ?? item).type === "FunctionDeclaration" && (item.declaration ?? item).id?.name === name,
		);
		assert.ok(node, `Missing Jev function ${name}`);
		const scanner = createScanner(true, undefined, text.slice(node.start, node.end));
		const tokens = [];
...
test("Decider secret resolution reads only explicitly approved separate references", () => {
	...
	try {
		...
		assert.throws(
			() => resolveHostSecrets({ secretRefs: ["env:TYPESAFE_API_KEY"] }),
			toolError("host_configuration"),
		);
		...
		if (old === undefined) delete process.env.ATOMIC_DECIDER_SERVICE_TOKEN;
		else process.env.ATOMIC_DECIDER_SERVICE_TOKEN = old;
	}
});
test("deterministic evaluation baseline uses the same guarded action and output budget without inference", {
	skip: !linux,
}, async () => {
	...
	try {
		const result = await deterministicBatchedRetrieval(
			{ objective: "Find answer", seedLocations: [{ path: "src/a.ts" }] },
...
test("lowered read/term limits remain respected for observed references", () => {
	...
	const builder = new CandidateBuilder(
		{ objective: "x", seedLocations: [{ path: "src/a.ts", line: 100 }] },
		scope(),
		p,
	);
	const action = builder.build()[0];
	assert.equal(action.action.startLine, 91);
	assert.equal(action.action.endLine, 100);
	builder.update(
		action,
		{ kind: "source_excerpt", text: "ReferenceError: LongIdentifier", sources: [] },
		{
			id: "e-observed",
			kind: "source_excerpt",
			text: "ReferenceError: LongIdentifier",
			contentHash: hash("x"),
			actionId: action.id,
			bounded: true,
		},
...
test("host secret-filter failures fail closed without echoing private exceptions", () => {
	const filter = createSecretScreen([], () => {
		throw new Error("private host filter diagnostic");
	});
	assert.equal(filter("ordinary text"), false);
});

test("trace inspection selects the latest terminal without reordering evidence", () => {
	const path = join(built, "inspection.jsonl");
	const entries = [
		{ event: "evidence", id: "first" },
		{ event: "handoff", status: "returned" },
		{ event: "evidence", id: "second" },
		{ event: "error", status: "cancelled" },
	];
	writeFileSync(path, `${entries.map(JSON.stringify).join("\n")}\n{"incomplete":`);
	const result = inspectTrace(path);
	assert.equal(result.status, "cancelled");
	assert.deepEqual(result.terminal, entries[3]);
	assert.deepEqual(result.evidence, [entries[0], entries[2]]);
	writeFileSync(path, "");
	assert.deepEqual(inspectTrace(path), { status: "incomplete", evidence: [] });
});

test("operation deadlines reject late results even before the timer gets an event-loop turn", async () => {
	let now = 0;
	const budget = new InvocationBudget(() => now, new AbortController().signal, 15000);
	await assert.rejects(
		budget.run(2000, async () => {
			now = 2001;
			return "late";
		}),
		Handoff,
...
test("operation deadlines reject dispatch after a synchronous hook exhausts its local budget", async () => {
	let now = 0;
	let dispatched = false;
	const repository = mockRepository(scope(), {
		async execute(_candidate, context) {
			now = 2001;
			context.markDispatched();
			dispatched = true;
	...
	const { result } = await run(
		{ objective: "gather", seedLocations: [{ path: "src/a.ts" }] },
		{ repository, now: () => now },
	);
	assert.equal(result.reason, "deadline");
	assert.equal(result.counters.actionsDispatched, 0);
...
test("Decider adapter enforces the dynamic Choice schema with installed TypeBox", async () => {
	const { inferDeciderDecision } = await load("structured-output/decider.js");
	const ids = ["a_dynamic", "handoff"];
	const schema = Type.Object(
		{ actionId: Type.Unsafe(Type.Union(ids.map((id) => Type.Literal(id)))) },
		{ additionalProperties: false },
	);
	for (const selected of ids) {
		const request = {
			state: { objective: "gather evidence" },
			schema,
			instructions: "Choose read or handoff.",
			jev: {
				questions: {
					next: { instructions: "Choose.", criteria: { a_dynamic: "Read source.", handoff: "Return control." } },
				},
				...
				new Response(
					JSON.stringify(
						response(
							ids,
							selected,
							ids.map((id) => (id === selected ? 1 : 0)),
						),
					),
				),
		});
		assert.deepEqual((await inferDeciderDecision(request, service, "candidate-hash")).value, { actionId: selected });
		await assert.rejects(
			inferDeciderDecision(
				{ ...request, jev: { ...request.jev, decode: () => ({ actionId: "invented" }) } },
				service,
				"candidate-hash",
			),
			Handoff,
...
test("an unavailable OS account leaves the feature off without consulting HOME", () => {
	const ACCOUNT_LOOKUP_PROCESS_TIMEOUT_MS = 10_000;
	const child = spawnSync(
		...
		os.userInfo = () => { throw new Error("OS account unavailable"); };
		syncBuiltinESMExports();
		process.env.HOME = ${JSON.stringify(built)};
		const policy = await import(${JSON.stringify(pathToFileURL(join(built, "investigation/host-policy.js")).href)});
		assert.equal(policy.USER_HOST_CONFIG_PATH, undefined);
		const { createHostInvestigationTool } = await import(${JSON.stringify(pathToFileURL(join(built, "investigation/host.js")).href)});
		assert.equal(createHostInvestigationTool({ _cwd: ${JSON.stringify(root)} }), undefined);
	`,
		],
		{ encoding: "utf8", timeout: ACCOUNT_LOOKUP_PROCESS_TIMEOUT_MS },
	);
