import { closeSync, constants, fstatSync, fsyncSync, openSync, readFileSync, realpathSync, writeSync } from "node:fs";
...
export class FileTraceSink implements TraceSink {
	...
	constructor(directory: string, repositoryRoot: string, invocationId: string, isSafe: (text: string) => boolean) {
		...
		this.isSafe = isSafe;
		if (!/^[a-zA-Z0-9_-]{1,128}$/.test(invocationId) || withinRoot(repositoryRoot, directory))
		...
		try {
			...
			try {
				...
				this.fd = openSync(
					`/proc/self/fd/${dir}/${invocationId}.jsonl`,
...
export function inspectTrace(path: string): {
	status: "incomplete" | "cancelled" | "returned";
	evidence: JsonObject[];
	terminal?: JsonObject;
} {
	...
	const terminal = entries
		.slice()
		.reverse()
		.find((item) => item.event === "handoff" || item.event === "error");
	return {
		status:
			terminal?.status === "returned" ? "returned" : terminal?.status === "cancelled" ? "cancelled" : "incomplete",
		evidence: entries.filter((item) => item.event === "evidence"),
		...(terminal ? { terminal } : {}),
	};
}
