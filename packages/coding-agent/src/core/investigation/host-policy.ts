...
import { createSecretScreen, normalizePath, parseLimits, parseProfile, validateEndpoint, withinRoot } from "./policy.js";
...
export const USER_HOST_CONFIG_PATH = join(userInfo().homedir, ".atomic", "decider-investigation.json");
...
export function readHostConfiguration(path: string | undefined, repositoryRoot: string): HostInvestigationConfiguration | undefined {
	...
	try {
		...
		try {
			...
			if (!before.isFile() || before.size > 65536n || before.uid !== BigInt(process.getuid?.() ?? -1) || (before.mode & 0o077n) !== 0n || withinRoot(realpathSync(repositoryRoot), actual)) throw new Error();
			raw = readFileSync(fd, "utf8");
			const after = fstatSync(fd, { bigint: true });
			if (before.ino !== after.ino || before.size !== after.size || before.mtimeNs !== after.mtimeNs || before.ctimeNs !== after.ctimeNs) throw new Error();
	...
	let value: unknown;
	try { value = parseStrictJson(raw); } catch { throw new InvestigationToolError("host_configuration"); }
	if (!record(value) || !exactKeys(value, ["experimental"]) || !record(value.experimental) || !exactKeys(value.experimental, ["deciderInvestigation"]) || !record(value.experimental.deciderInvestigation)) throw new InvestigationToolError("host_configuration");
	const config = value.experimental.deciderInvestigation;
	if (config.enabled === false) return undefined;
	if (config.enabled !== true || !exactKeys(config, ["enabled", "endpoint", "backendId", "policyProfileId", "profile", "tokenSecretRef", "approvedRepositories"], ["limits", "secretRefs"])) throw new InvestigationToolError("host_configuration");
	const profile = parseProfile(config.profile);
	if (profile.backendId !== config.backendId || profile.id !== config.policyProfileId || /^fixture(?:[-:/]|$)/i.test(profile.evaluationId)) throw new InvestigationToolError("host_configuration");
	const endpoint = validateEndpoint(config.endpoint);
	const limits = parseLimits(config.limits);
	if (config.tokenSecretRef !== "env:ATOMIC_DECIDER_SERVICE_TOKEN") throw new InvestigationToolError("host_configuration");
	const secretRefs = config.secretRefs ?? [];
	if (!Array.isArray(secretRefs) || secretRefs.length > 64 || secretRefs.some((ref) => typeof ref !== "string" || !/^env:[A-Z][A-Z0-9_]*$/.test(ref) || ref === "env:TYPESAFE_API_KEY")) throw new InvestigationToolError("host_configuration");
	if (!Array.isArray(config.approvedRepositories) || config.approvedRepositories.length > 64) throw new InvestigationToolError("host_configuration");
	let permitted: { root: string; scopePaths: string[]; excludedPaths: string[] } | undefined;
	for (const entry of config.approvedRepositories) {
		if (!record(entry) || !exactKeys(entry, ["root", "scopePaths"], ["excludedPaths"]) || typeof entry.root !== "string" || !isAbsolute(entry.root) || entry.root !== resolve(entry.root) || !Array.isArray(entry.scopePaths) || !entry.scopePaths.length || entry.scopePaths.length > 64 || (entry.excludedPaths !== undefined && (!Array.isArray(entry.excludedPaths) || entry.excludedPaths.length > 64))) throw new InvestigationToolError("host_configuration");
	...
	return freeze({ sourceHash: hash(raw), ...permitted, endpoint, profile, limits, tokenSecretRef: config.tokenSecretRef, secretRefs: secretRefs as string[] });
}
export function resolveHostSecrets(config: HostInvestigationConfiguration, hostFilter?: (text: string) => boolean) {
	...
	if (!token || Buffer.byteLength(token) < 32 || /[\r\n]/.test(token)) throw new InvestigationToolError("host_configuration");
	const values = [token];
