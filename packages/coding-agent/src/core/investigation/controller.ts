...
export function validateSelection(
	...
	if (
		...
		!exactKeys(selection.value, ["actionId"]) ||
		typeof selection.value.actionId !== "string" ||
		!ids.includes(selection.value.actionId) ||
		!record(selection.usage) ||
		!exactKeys(selection.usage, ["inputTokens", "outputTokens"]) ||
		!nonnegative(selection.usage.inputTokens) ||
		!nonnegative(selection.usage.outputTokens)
	)
		throw new Handoff("decision_invalid");
	const evidence = selection.evidence;
	if (
		!record(evidence) ||
		evidence.selectedId !== selection.value.actionId ||
		!record(evidence.rawProbabilities) ||
		!record(evidence.probabilities) ||
		!exactKeys(evidence.rawProbabilities, ids) ||
		!exactKeys(evidence.probabilities, ids)
	)
		throw new Handoff("decision_invalid");
	if (
		evidence.backendId !== deps.policy.profile.backendId ||
		evidence.deploymentFingerprint !== deps.policy.profile.deploymentFingerprint ||
		evidence.questionVersion !== deps.policy.profile.questionVersion
	)
		throw new Handoff("model_mismatch");
	if (evidence.candidateSetHash !== candidateSetHash || evidence.normalization !== "decider-four-decimal")
		throw new Handoff("decision_invalid");
	...
	for (const id of ids)
		if (
			!probability(evidence.probabilities[id]) ||
			Math.abs(evidence.probabilities[id] - evidence.rawProbabilities[id] / sum) > 1e-12
		)
			throw new Handoff("decision_invalid");
	const sorted = Object.values(evidence.probabilities).sort((a, b) => b - a);
	if (
		evidence.probabilities[evidence.selectedId] !== sorted[0] ||
		evidence.topProbability !== sorted[0] ||
		Math.abs(evidence.topTwoMargin - (sorted[0] - sorted[1])) > 1e-12
	)
		throw new Handoff("decision_invalid");
	if (
		(evidence.providerConfidence !== undefined &&
			(!probability(evidence.providerConfidence) ||
				evidence.providerConfidence !== evidence.rawProbabilities[evidence.selectedId])) ||
		(evidence.providerCertainty !== undefined && !probability(evidence.providerCertainty))
	)
		throw new Handoff("decision_invalid");
	return evidence;
}
export function validateObservation(
	...
	if (
		...
		bytes(observation.text) > remainingBytes ||
		typeof observation.bounded !== "boolean" ||
		!Array.isArray(observation.sources) ||
		!nonnegative(observation.omittedEvidenceBytes) ||
		(observation.omittedMatches !== null && !nonnegative(observation.omittedMatches))
	)
		throw new InvestigationToolError("contract");
	for (const source of observation.sources)
		if (
			!Object.hasOwn(scope.files, source.path) ||
			source.version !== scope.files[source.path].version ||
			!/^[a-f0-9]{64}$/.test(source.contentHash) ||
			!nonnegative(source.bytesRead)
		)
			throw new Handoff("source_changed");
	if (candidate.action.kind === "read_range") {
		if (
			observation.kind !== "source_excerpt" ||
			observation.path !== candidate.action.path ||
			observation.startLine !== candidate.action.startLine ||
			typeof observation.endLine !== "number" ||
			observation.endLine > candidate.action.endLine ||
		...
		if (
			observation.kind !== "search_matches" ||
			!Array.isArray(observation.matches) ||
			typeof observation.scopeFullyScanned !== "boolean" ||
			(!observation.scopeFullyScanned && observation.omittedMatches !== null)
		)
			throw new InvestigationToolError("contract");
		for (const match of observation.matches)
			if (
				!Object.hasOwn(scope.files, match.path) ||
				!Number.isSafeInteger(match.line) ||
				match.line <= 0 ||
				typeof match.text !== "string" ||
				typeof match.clipped !== "boolean"
			)
				throw new InvestigationToolError("contract");
	}
}
export async function investigateCode(
	...
	const history: Record<string, { actionId: string; kind: string; evidenceId: string | null; newEvidence: boolean }> =
		Object.create(null);
	const audit = (event: string, data: object) => {
		try {
			deps.trace.write(event, jsonObject({ invocationId: deps.invocationId, elapsedMs: budget.elapsed(), ...data }));
		} catch {
	...
	try {
		...
		audit("invocation-start", {
			status: "incomplete",
			policyGeneration: deps.policy.generation,
			profile: deps.policy.profile,
			limits,
		});
		if (!deps.policy.isSafe(canonical(input))) return finish("input_context_unsafe");
		const scope = freeze(
			await budget.run(limits.operationTimeoutMs, (childSignal) => deps.repository.prepare(childSignal)),
		);
		active();
		if (!deps.policy.isSafe(canonical({ paths: Object.keys(scope.files), restrictions: scope.restrictions })))
			return finish("input_context_unsafe");
		const builder = new CandidateBuilder(input, scope, deps.policy);
		const fresh = async () => {
			active();
			await budget.run(limits.operationTimeoutMs, (childSignal) => deps.repository.assertFresh(scope, childSignal));
			active();
		};
		for (;;) {
			active();
			...
			if (candidates.length > limits.maxCandidates) {
				omitted.candidates = candidates.length;
				return finish("candidate_overflow");
			}
			if (counters.actionsDispatched >= limits.maxActions || counters.decisionRequests >= limits.maxDecisionRequests)
			...
			let state = jsonObject({
				...
				budgets: {
					actions: limits.maxActions - counters.actionsDispatched,
					decisions: limits.maxDecisionRequests - counters.decisionRequests,
					timeMs: budget.remaining(),
					enumeratedEntries: deps.repository.accounting().entriesEnumerated,
					bytesScanned: deps.repository.accounting().bytesScanned,
					scanBytesRemaining: limits.maxScannedBytes - deps.repository.accounting().bytesScanned,
					evidenceBytes: limits.maxEvidenceBytes - evidenceBytes,
					limits,
				},
				omitted,
			});
			...
			const selection = await budget.run(limits.decisionTimeoutMs, (childSignal) =>
				deps.decide(freeze({ state, candidates, candidateSetHash, stateHash }), childSignal),
			);
			active();
			...
			if (
				choice.topTwoMargin <= 0 ||
				choice.topProbability < deps.policy.profile.minTopProbability ||
				choice.topTwoMargin < deps.policy.profile.minTopTwoMargin
			)
			...
			try {
				observation = await budget.run(limits.operationTimeoutMs, (childSignal) =>
					deps.repository.execute(candidate, {
						...
						markDispatched: () => {
							...
							if (dispatched || counters.actionsDispatched >= limits.maxActions)
								throw new InvestigationToolError("contract");
							audit("action-start", {
								operationId,
								...
								profileId: deps.policy.profile.id,
							});
							active();
							childSignal.throwIfAborted();
				...
				audit("action-result", {
					...
					reason: error instanceof Handoff ? error.reason : "tool_error",
				});
				throw error;
			}
			...
			const duplicate = evidence.find(
				(item) =>
					...
					item.startLine === observation.startLine &&
					item.contentHash === contentHash,
			);
			let added: Evidence | undefined;
			if (!duplicate && observation.text.length > 0) {
				added = {
					id: `e_${hash(canonical({ kind: observation.kind, path: observation.path ?? null, startLine: observation.startLine ?? null, contentHash })).slice(0, 32)}`,
			...
			audit("action-result", {
				...
				dispatched: true,
				status: "ok",
				evidenceId: added?.id ?? duplicate?.id ?? null,
				scopeFullyScanned: observation.scopeFullyScanned ?? null,
				sources: observation.sources,
				omittedMatches: observation.omittedMatches,
				omittedEvidenceBytes: observation.omittedEvidenceBytes,
			});
		...
		audit("error", {
			...
			reason: error instanceof InvestigationToolError ? error.code : "tool_error",
			counters,
			evidenceIds: evidence.map((item) => item.id),
		});
