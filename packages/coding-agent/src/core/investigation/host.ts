...
import { readHostConfiguration, resolveHostSecrets, USER_HOST_CONFIG_PATH } from "./host-policy.js";
...
const inputSchema = Type.Object(
	{
		objective: Type.String({
			minLength: 1,
			description: "Information to gather. This does not authorize edits or arbitrary tools.",
		}),
		diagnosticText: Type.Optional(Type.String()),
		seedLocations: Type.Optional(
			Type.Array(
				Type.Object(
					{ path: Type.String(), line: Type.Optional(Type.Integer({ minimum: 1 })) },
					{ additionalProperties: false },
				),
				{ maxItems: 8 },
			),
		),
		literalTerms: Type.Optional(
			Type.Array(Type.String({ description: "A literal term of 2–128 Unicode characters; not a regex." }), {
				maxItems: 8,
			}),
		),
	},
...
export function createHostInvestigationTool(
	session: AgentSession,
	hostSecretFilter?: (text: string) => boolean,
): ToolDefinition<typeof inputSchema, InvestigationResult> | undefined {
	...
	return {
		...
		label: "Investigate code (experimental)",
		description:
			"Delegate one bounded, read-only repository investigation. Returns source evidence, not a diagnosis or completed coding task. A local Decider selects up to five already-constructed reads/searches; the main model continues normally.",
		...
		async execute(toolCallId, input, signal) {
			...
			try {
				...
				if (!config || config.sourceHash !== initial.sourceHash)
				...
				const assertCurrent = () => {
					...
					if (
						!live ||
						live.sourceHash !== config.sourceHash ||
						session._cwd !== config.root ||
					...
					for (const name of ["read", "search", "investigate_code"])
						if (
							!names.includes(name) ||
							session._excludedToolNames?.has(name) ||
							(session._allowedToolNames && !session._allowedToolNames.has(name))
						)
							throw new InvestigationToolError("permission_denied");
					if (
						session._toolDefinitions.get("read")?.definition !== readDefinition ||
						session._toolDefinitions.get("search")?.definition !== searchDefinition ||
						session._toolDefinitions.get("investigate_code")?.definition !== owningDefinition ||
						resolveHostSecrets(live, hostSecretFilter).signature !== secrets.signature
					)
						throw new InvestigationToolError("permission_denied");
				};
				assertCurrent();
				...
				const repository = new LinuxRepositoryFacade({
					...
					policy,
					trace,
					dispatch: createInvestigationDispatcher(session, assertCurrent),
				});
				const backend = Object.freeze({
					...
					isSafe: secrets.isSafe,
				});
				const result = await investigateCode(
					input,
					{
						...
						decide: async (step, childSignal) => {
							...
							const schema = Type.Object(
								{ actionId: Type.Unsafe<string>(Type.Union(ids.map((id) => Type.Literal(id)))) },
								{ additionalProperties: false },
							);
							const decision = await inferDeciderDecision(
								{
									state: step.state,
									schema,
