...
export function createInvestigationDispatcher(session: AgentSession, assertCurrent: () => void): GuardedRepositoryDispatcher {
	return async (call, execute, signal) => {
		...
		const metadata = { toolName: call.toolName, toolCallId: call.operationId, input: call.args, parentInvocationId: call.parentInvocationId, operationId: call.operationId };
		try {
			const permission = runner.hasHandlers("tool_call") ? await emitGuardedToolCall(runner, { type: "tool_call", ...metadata }) : undefined;
			active();
		...
		let observation;
		try { observation = await execute(); }
		catch (error) {
			// Result hooks see dispatched failures as well, but never private exception text.
			if (runner.hasHandlers("tool_result")) await emitGuardedToolResult(runner, { type: "tool_result", ...metadata, content: [{ type: "text", text: "Investigation repository operation failed." }], details: {}, isError: true }, true);
			throw error;
		}
		active();
		...
		const filtered = runner.hasHandlers("tool_result") ? await emitGuardedToolResult(runner, { type: "tool_result", ...metadata, content, details: observation, isError: false }, true) : undefined;
		active();
		if (filtered?.isError || canonical(filtered?.content ?? content) !== contentBefore || canonical(filtered?.details ?? observation) !== detailsBefore) {
