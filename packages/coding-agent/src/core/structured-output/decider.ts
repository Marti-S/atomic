...
import { requestDecider, type ResolvedDeciderBackend } from "./decider-transport.js";
...
export async function inferDeciderDecision<T extends TSchema>(
	...
	if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0 || timeoutMs > backend.limits.decisionTimeoutMs) throw new Handoff("decision_invalid");
	...
	try {
		...
		const result = await abortable(requestDecider(state, questions, request.instructions, backend, candidateSetHash, controller.signal), controller.signal);
		...
		controller.signal.throwIfAborted();
		return { value, model: backend.profile.backendId, responseModel: backend.profile.backendId, usage: result.usage, evidence: result.evidence };
