...
| `npm run test:integration` | Frozen code exit 1, 1157 passed, 12 skipped, 4 failed; `test-integration-closure.log`. Packed consumer output byteCount 4 versus 14, one prompt-node 30s timeout and two retained-workflow 60s subprocess timeouts. Required installed-package gate is red and cause unclassified. |
...
The root metadata and package system-prompt tests still require `docs/mcp.md`, but that path was removed by the required base's merged documentation PR #3119. This is an unrelated base code/test mismatch, not an environmental failure, and no test or base documentation was rewritten to hide it. Final installed-package parity is separately unclassified: `test/fixtures/consumer-parity.mjs:371` expects `Buffer.byteLength("child complete")` (14), but task record output byteCount is 4 after its completed-result assertion passes. It is an objective-relevant unresolved failure, not a justified blocker. Qlty's host complexity is deferred as out-of-scope refactoring.
